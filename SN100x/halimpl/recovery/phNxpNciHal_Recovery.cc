 ...
 *
 ******************************************************************************/
static NFCSTATUS phNxpNciHal_ReadResponse(uint16_t *len, uint8_t **rsp_buffer, long timeout) {
  NFCSTATUS status = NFCSTATUS_FAILED;
...
static NFCSTATUS phnxpNciHal_partialOpen(void) {
  ...
  if (nfc_dev_node == NULL) {
    ...
    NXPLOG_NCIHAL_D(
        ...
        "/dev/pn54x");
    strlcpy(nfc_dev_node, "/dev/pn54x", (NXP_MAX_CONFIG_STRING_LEN * sizeof(char)));
  }
